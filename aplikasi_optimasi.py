...
# ---------- Tab Layout ----------
tabs = st.tabs(["🔧 Analisis Produksi", "📈 Optimasi Produksi"])

# ---------- Tab 1: Analisis Produksi ----------
with tabs[0]:
    st.title("🔧 Analisis Produksi - Mesin & Operator")
...
with tabs[0]:
    ...
    styled_df = df_vertikal.style.set_properties(**{'text-align': 'left'})
...
# ---------- Tab 2: Optimasi Produksi ----------
with tabs[1]:
    st.title("📈 Optimasi Produksi Berdasarkan Keuntungan Maksimum")
    total_operator_tersedia = st.number_input("Masukkan Total Operator yang Tersedia", min_value=1, value=10)

    st.markdown("**Model LP:** Maksimalkan total keuntungan dengan batasan jumlah operator yang tersedia")
    ...
    if LpStatus[model.status] == "Optimal":
        ...
        st.error("❌ Tidak ada solusi optimal ditemukan. Periksa kembali input dan batasan operator.")
